...
public class Script {
    ...
    public List<DataVariable> getDataVaraibles() {
        return dataVaraibles;
    }

    public void setDataVaraibles(List<DataVariable> dataVaraibles) {
        this.dataVaraibles = dataVaraibles;
    }
    ...
    public void setCategoryBlocks(List<String> categoryBlocks) {
    ...
    public Script(Object  scriptContent)
    {
        ...
        List<String> changevar = getallDataVariables("changeVar");
        List<String> hidevar = getallDataVariables("hideVariable");
        List<String> showvar = getallDataVariables("showVariable");
        List<String> setvar = getallDataVariables("setVar");
    ...
    private List<String> fillCateogryBlocks()
    {
        ...
        List<List<String>> categories = map.getCategories();
        int count = 0;
        int foundPosition = 0;
        String scriptString = this.scriptContent.toString();
        
        for (int i = 0; i < categories.size(); i++)
        {
            for (int j = 0; j < categories.get(i).size(); j++)
            {
                foundPosition = scriptString.indexOf(categories.get(i).get(j),foundPosition);
                
                while (foundPosition != -1)
                {
                   count = count + 1; 
                   foundPosition = scriptString.indexOf(categories.get(i).get(j),foundPosition + 1);
                }
            }
          
            
            if(i == 0)
                auxcategoryBlocks.add("Control, " + count);
            else if (i == 1)
                auxcategoryBlocks.add("Event, " + count);
            else if (i == 2)
               auxcategoryBlocks.add("Looks, " + count);
            else if (i == 3)
                auxcategoryBlocks.add("Motion, " + count);
            else if (i == 4)
                auxcategoryBlocks.add("Operator, " + count);
            else if (i == 5)
                auxcategoryBlocks.add("Pen, " + count);
            else if (i == 6)
                auxcategoryBlocks.add("Sensing, " + count);
            else if (i == 7)
                auxcategoryBlocks.add("Sound, " + count);
            count = 0;
        }
    ...
    private List<String> getallDataVariables(String word)
    {
        ...
        String scriptsString = script.toString();
